...
To get the first value: (7 + 4 + 0 + 5 + 6 + 2 + 6 + 10 + 7) = 47 / 9 = 5.2222 = 5. The other three integers are obtained the same way, 
...
namespace BoxBlur
{
    class Program
    {
        ...
        {
            int[][] blurredBox = new int[image.Length - 2][];
            for (int i = 0; i < image.Length - 2; i++)
            {
                blurredBox[i] = new int[image[0].Length - 2];
            }
            int sum;
            for (int i = 0; i < image.Length - 2; i++)
            {
